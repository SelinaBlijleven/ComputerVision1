...
% the sphere can be modeled correctly.
subplot(2, 2, 1);
...
% are correctly modeled. In the image it possible to see that the source
% vector is correctly modeled, as the complete image has the pixelvalues ranging 
% from zero to one.
subplot(2, 2, 2);
imshow(albedo);
title('Albedo');
...
% images that have been input into the function. The normals show the way
% the light is reflected of the object if the light source is directly on
% top of the surface patch.
